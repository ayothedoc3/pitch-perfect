...
const cleanupUploadedFiles = (paths: Array<string | undefined>) => {
...
router.post('/:id/upload', authenticateToken, mediaUpload, async (req: AuthRequest, res) => {
  try {
    const { id } = req.params;
    const userId = req.user!.id;
    ...
    if (!pitch) {
      cleanupUploadedFiles([audioFile?.path, videoFile?.path]);
      return res.status(404).json({ error: 'Pitch not found' });
    }

    const mediaForAnalysisPath = (audioFile?.path || videoFile?.path)!;

    const updatedPitch = await prisma.pitch.update({
      where: { id },
      data: {
        audioUrl: audioFile ? `/uploads/${audioFile.filename}` : undefined,
        videoUrl: audioFile && audioFile.mimetype.startsWith('video/')
    ...
    setImmediate(async () => {
      try {
        ...
        const analysisResult = await speechAnalysis.analyzeComplete(mediaForAnalysisPath, pitch.duration);
    ...
    const files = (req.files as Record<string, Express.Multer.File[]> | undefined) || {};
    cleanupUploadedFiles([files.file?.[0]?.path, files.video?.[0]?.path]);
